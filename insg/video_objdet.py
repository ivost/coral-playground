...
def main():
    default_model_dir = '../models/mobilenet'
    ...
    convert_to_mp4(outp, mp4)
    print("DONE " + mp4)


def convert_to_mp4(inp, outp):
    import subprocess
    assert os.path.exists(inp)
    print(f"converting {inp} to {outp}")
...
def append_objs_to_img(cv2_im, inference_size, objs, labels):
    ...
    for obj in objs:
        ...
        cv2_im = cv2.putText(cv2_im, label, (x0, y0 + 30), cv2.FONT_HERSHEY_SIMPLEX, 1.0, color, 2)
    return cv2_im
